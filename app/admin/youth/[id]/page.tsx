...
export default async function YouthDetailPage({ params }: { params: { id: string } }) {
  ...
  const { data: plan } = await supabase.from('success_plans').select('*').eq('user_id', params.id).eq('status', 'active').single()
  const { data: milestones } = plan
    ? await supabase.from('milestones').select('*').eq('plan_id', plan.id).order('sort_order')
    : { data: [] }
  const { data: mentorRequest } = await supabase.from('mentor_requests').select('*').eq('youth_id', params.id).order('created_at', { ascending: false }).limit(1).single()
